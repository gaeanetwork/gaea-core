...
func CheckArgsContainsHashAndSignatures(args []string, pubkey string) error {
	...
	if hash1 := common.BytesToHex(GetArgsHashBytes(args[:hashIndex])); hash != hash1 {
...
func GetArgsHashAndSignatures(privHex string, args []string) ([]byte, []byte, error) {
	hash := GetArgsHashBytes(args)
	signature, err := ecc.SignECDSA(privHex, hash)
	if err != nil {
		return nil, nil, err
	}
